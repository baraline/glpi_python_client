...
The tests stub the API methods on a real client so the statistics
aggregations exercise their real summarization logic without any network
call.
"""
...
from glpi_python_client._async.clients.custom._statistics import (
    _V1_SO_ASSIGNEE,
    _V1_SO_REQUESTER,
)
...
async def test_get_ticket_statistics_rejects_invalid_window(client: Any) -> None:
...
async def test_get_ticket_statistics_rejects_malformed_iso_date(
    client: Any,
) -> None:
    ...
    with pytest.raises(GlpiValidationError, match="start_date") as excinfo:
        await client.get_ticket_statistics(
            start_date="2026-13-45", end_date="2026-01-31"
        )
    assert isinstance(excinfo.value, ValueError)
    assert isinstance(excinfo.value.__cause__, ValueError)


async def test_get_task_statistics_zero_for_empty_input(client: Any) -> None:
...
async def test_get_ticket_statistics_entity_id_filter(client: Any) -> None:
...
async def test_get_ticket_statistics_entity_name_resolution(client: Any) -> None:
...
async def test_get_ticket_statistics_entity_name_no_match(client: Any) -> None:
...
async def test_get_ticket_statistics_extra_filter_appended(client: Any) -> None:
...
async def test_get_ticket_statistics_default_days_window(client: Any) -> None:
    ...
    client.search_tickets = fake_search  # type: ignore[method-assign]
    await client.get_ticket_statistics(default_days=14)
    end = date.today()
...
async def test_get_task_durations_empty_ticket_list(client: Any) -> None:
    """When no tickets match, all durations are zero."""

    async def fake_iter(rsql_filter: str = "", *, batch_size: int = 200):
        for batch in []:
            yield batch
...
async def test_get_task_durations_entity_grouping(client: Any) -> None:
    ...
    async def fake_task_stats(ticket_ids: list[int]) -> dict[str, Any]:
        return {
            "ticket_count": 2,
            "task_count": 2,
            "total_duration": 1200,
...
async def test_get_task_durations_return_task_details_true(client: Any) -> None:
...
async def test_get_task_durations_return_task_details_false(client: Any) -> None:
...
async def test_get_user_activity_raises_without_identifier(client: Any) -> None:
...
async def test_get_user_activity_single_user_happy_path(client: Any) -> None:
...
async def test_get_user_activity_raises_when_no_users_matched(client: Any) -> None:
...
async def test_get_user_activity_multi_user_merge(client: Any) -> None:
...
async def test_v1_task_statistics_matches_the_per_ticket_shape(client: Any) -> None:
    ...
    result = await client._v1_task_statistics([10, 11], since=date(2026, 7, 1))
...
async def test_v1_task_statistics_stops_paging_before_the_window(
    client: Any,
) -> None:
    ...
    result = await client._v1_task_statistics([10], since=date(2026, 7, 1))
...
async def test_get_task_durations_uses_per_ticket_path_below_threshold(
    client: Any,
) -> None:
...
async def test_get_ticket_statistics_counts_beyond_one_page(client: Any) -> None:
    ...
    result = await client.get_ticket_statistics(default_days=7)

    assert calls == [0, 200]
    assert result["entities"]["1"]["total"] == 230


async def test_get_ticket_statistics_resolves_entities_beyond_one_page(
    client: Any,
) -> None:
    ...
    await client.get_ticket_statistics(default_days=7, entity_name="Acme")

    assert "entity.id==201" in captured_tickets["filter"]


async def test_get_task_durations_resolves_entities_beyond_one_page(
    client: Any,
) -> None:
    ...
    await client.get_task_durations(default_days=7, entity_name="Acme")
