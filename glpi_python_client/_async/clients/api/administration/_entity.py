"""GLPI ``/Administration/Entity`` mixin.
...
from glpi_python_client._async.clients.commons._constants import ENTITY_ENDPOINT, GlpiId
...
class EntityMixin(TransportMixin):
    """CRUD helpers for ``/Administration/Entity``."""
    ...
    async def iter_search_entities(
        ...
        *,
        ...
        start = 0
        while True:
            batch = await self.search_entities(
                rsql_filter,
                limit=batch_size,
                start=start,
            )
