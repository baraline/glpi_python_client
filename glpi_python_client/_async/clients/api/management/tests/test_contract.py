...
async def test_update_contract_cost(client: Any) -> None:
    """``update_contract_cost`` patches the per-cost endpoint."""

    rec = TransportRecorder()
    rec.install(client)
    await client.update_contract_cost(9, 3, PatchContractCost(cost=1500.0))
    assert rec.calls[0]["endpoint"] == "Management/Contract/9/Cost/3"


async def test_delete_contract_cost_with_force(client: Any) -> None:
    """``delete_contract_cost(force=True)`` ships the force flag."""

    rec = TransportRecorder()
    rec.install(client)
    await client.delete_contract_cost(9, 3, force=True)
    assert rec.calls[0]["endpoint"] == "Management/Contract/9/Cost/3"
    assert rec.calls[0]["json"]["force"] is True


async def test_contract_cost_id_is_the_only_readonly_field() -> None:
