...
async def test_stream_document_content_yields_chunks(client: Any) -> None:
    ...
    chunks = [c async for c in client.stream_document_content(7, chunk_size=4)]
...
async def test_stream_document_content_raises_on_error_status(client: Any) -> None:
    ...
    with pytest.raises(GlpiStatusError):
        [c async for c in client.stream_document_content(7)]
