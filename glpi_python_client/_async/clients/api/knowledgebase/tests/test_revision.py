...
async def test_list_kb_article_revisions_default_language(client: Any) -> None:
    rec = _Recorder(get_payload=[{"id": 11, "revision": 2}])
    rec.install(client)
    revisions = await client.list_kb_article_revisions(5)
    assert revisions[0].revision == 2
    assert rec.calls[0]["endpoint"] == "Knowledgebase/Article/5/Revision"


async def test_list_kb_article_revisions_with_language_uses_path_segment(
    client: Any,
) -> None:
    rec = _Recorder(get_payload=[{"id": 11, "revision": 2}])
    rec.install(client)
    await client.list_kb_article_revisions(5, language="fr_FR")
    assert rec.calls[0]["endpoint"] == "Knowledgebase/Article/5/fr_FR/Revision"


async def test_get_kb_article_revision_default_language(client: Any) -> None:
    rec = _Recorder(get_payload={"id": 11, "revision": 2})
    rec.install(client)
    revision = await client.get_kb_article_revision(5, 2)
    assert revision.revision == 2
    assert rec.calls[0]["endpoint"] == "Knowledgebase/Article/5/Revision/2"


async def test_get_kb_article_revision_with_language(client: Any) -> None:
    rec = _Recorder(get_payload={"id": 11, "revision": 2})
    rec.install(client)
    await client.get_kb_article_revision(5, 2, language="fr_FR")
