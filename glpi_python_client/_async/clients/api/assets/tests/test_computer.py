...
async def test_iter_search_computers_stops_on_short_page(client: Any) -> None:
    ...
    rec.install(client)
    pages = [page async for page in client.iter_search_computers(batch_size=3)]
    assert len(pages) == 1
...
async def test_iter_search_computers_yields_every_page(client: Any) -> None:
    ...
    batches = [
        batch async for batch in client.iter_search_computers("name==x", batch_size=3)
    ]
...
async def test_get_computer_endpoint(client: Any) -> None:
    """``get_computer`` hits the per-id endpoint."""

    rec = TransportRecorder(get_payload={"id": 9, "name": "LAPTOP-01"})
    rec.install(client)
    computer = await client.get_computer(9)
    assert computer.id == 9
    assert rec.calls[0]["endpoint"] == "Assets/Computer/9"


async def test_create_computer_returns_new_id(client: Any) -> None:
    """``create_computer`` posts the body and returns the server id."""

    rec = TransportRecorder(post_payload={"id": 31})
    rec.install(client)
    new_id = await client.create_computer(PostComputer(name="LAPTOP-02"))
    assert new_id == 31
    assert rec.calls[0]["endpoint"] == "Assets/Computer"
    assert rec.calls[0]["json"]["name"] == "LAPTOP-02"


async def test_update_computer(client: Any) -> None:
    """``update_computer`` patches the per-id endpoint."""

    rec = TransportRecorder()
    rec.install(client)
    await client.update_computer(9, PatchComputer(serial="SN-123"))
    assert rec.calls[0]["endpoint"] == "Assets/Computer/9"
    assert rec.calls[0]["json"]["serial"] == "SN-123"


async def test_delete_computer_with_force(client: Any) -> None:
    """``delete_computer(force=True)`` ships the force flag."""

    rec = TransportRecorder()
    rec.install(client)
    await client.delete_computer(9, force=True)
    assert rec.calls[0]["json"]["force"] is True


async def test_group_fields_parse_as_lists_of_references(client: Any) -> None:
...
async def test_post_computer_excludes_every_readonly_field() -> None:
