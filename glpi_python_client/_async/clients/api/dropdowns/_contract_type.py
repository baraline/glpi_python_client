...
class ContractTypeMixin(TransportMixin):
    ...
    async def search_contract_types(
        self,
        rsql_filter: str = "",
        *,
        limit: int = 50,
    ...
    async def iter_search_contract_types(
        self,
        rsql_filter: str = "",
        *,
        batch_size: int = 50,
    ) -> AsyncIterator[list[GetContractType]]:
        ...
        start = 0
        while True:
            batch = await self.search_contract_types(
                rsql_filter,
                limit=batch_size,
                start=start,
            )
    ...
    async def get_contract_type(self, contract_type_id: GlpiId) -> GetContractType:
    ...
    async def create_contract_type(self, contract_type: PostContractType) -> int:
