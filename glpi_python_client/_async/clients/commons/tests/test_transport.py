...
@pytest.fixture
async def transport_client() -> AsyncIterator[Any]:
...
async def test_ensure_token_calls_auth_manager(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_send_request_dispatches_through_session_request(
    ...
    async def _request(method: str, url: str, **kw: object) -> FakeResponse:
        seen.update({"method": method, "url": url, "kw": kw})
        return fake

    monkeypatch.setattr(c._session, "request", _request)
    result = await c._send_request(
...
async def test_send_request_does_not_use_per_verb_attributes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(c._session, "request", _request)
    result = await c._send_request("get", "https://glpi.example.test/api.php/v2/test")
    assert captured.get("used") is True
    assert result is not sentinel
    await c.close()


async def test_execute_request_get_builds_params(transport_client: Any) -> None:
    """``_execute_request`` places query params on GET requests."""

    captured: dict[str, Any] = {}

    async def _capture(method: str, url: str, **kw: Any) -> FakeResponse:
        captured.update({"method": method, "url": url, "kw": kw})
        return FakeResponse(status_code=200, payload={})

    transport_client._send_request = _capture  # type: ignore[method-assign]
    await transport_client._execute_request(
        method="get",
...
async def test_execute_request_post_builds_json_body(transport_client: Any) -> None:
...
async def test_get_request_returns_response(transport_client: Any) -> None:
    """``_get_request`` dispatches via ``_execute_request`` and returns the response."""

    resp = await transport_client._get_request("Assistance/Ticket")
    assert resp.status_code == 200


async def test_post_request_returns_response(transport_client: Any) -> None:
    ...
    resp = await transport_client._post_request(
        "Assistance/Ticket", json_body={"name": "t"}
    )
    assert resp.status_code == 201


async def test_update_request_returns_response(transport_client: Any) -> None:
    ...
    transport_client._send_request = _send  # type: ignore[method-assign]
    resp = await transport_client._update_request(
        "Assistance/Ticket/1", json_body={"name": "u"}
    )
    assert resp.status_code == 200


async def test_delete_request_returns_response(transport_client: Any) -> None:
    ...
    transport_client._send_request = _send  # type: ignore[method-assign]
    resp = await transport_client._delete_request("Assistance/Ticket/1")
...
def _no_retry_sleep(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for name in _RETRIED_METHODS:
        monkeypatch.setattr(getattr(AsyncGlpiClient, name).retry, "wait", wait_fixed(0))


@pytest.fixture
async def retry_client() -> AsyncIterator[Any]:
...
async def test_5xx_is_retried_three_times_and_reraises_server_error(
    ...
    with pytest.raises(GlpiServerError) as excinfo:
        await getattr(retry_client, method_name)("Assistance/Ticket")
...
async def test_persistent_5xx_does_not_surface_as_retry_error(
    retry_client: Any,
) -> None:
    ...
    retry_client._send_request = _send  # type: ignore[method-assign]
    with pytest.raises(GlpiServerError) as excinfo:
        await retry_client._get_request("Assistance/Ticket")
    assert not isinstance(excinfo.value, tenacity.RetryError)


@pytest.mark.parametrize("method_name", _RETRIED_METHODS)
async def test_4xx_is_not_retried_by_the_transport(
    retry_client: Any, method_name: str
) -> None:
    ...
    retry_client._send_request = _send  # type: ignore[method-assign]
    response = await getattr(retry_client, method_name)("Assistance/Ticket/1")
...
@pytest.mark.parametrize("method_name", _RETRIED_METHODS)
async def test_network_errors_are_still_retried(
    retry_client: Any, method_name: str
) -> None:
    ...
    async def _request(method: str, url: str, **kw: Any) -> FakeResponse:
        attempts.append(1)
...
@pytest.mark.parametrize("method_name", _RETRIED_METHODS)
async def test_no_third_party_exception_reaches_the_caller(
    retry_client: Any, method_name: str
) -> None:
...
async def test_timeouts_narrow_to_the_timeout_subclass(retry_client: Any) -> None:
