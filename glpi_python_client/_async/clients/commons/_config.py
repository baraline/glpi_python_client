"""Configuration and resource setup for the GLPI client.

The helpers here own environment parsing, URL normalisation, and the
construction of the runtime resources the client owns: the shared HTTP
session, the OAuth token manager, and the optional legacy v1 session.
"""
...
class SessionFactory(Protocol):
    ...
    def __call__(self, *, verify_ssl: bool) -> httpx.AsyncClient:
...
def build_http_session(*, verify_ssl: bool) -> httpx.AsyncClient:
...
def build_client_resources(
    *,
    ...
    from glpi_python_client._async.auth._v1_session import GLPIV1Session
    from glpi_python_client._async.auth.auth import (
        GLPITokenManager,
    ...
    # Validated before anything is constructed, so a bad configuration never
    # leaves a session to unwind. The previous shape -- build the session,
    # then close it in an ``except`` clause -- cannot work on the async
    # surface: an ``httpx.AsyncClient`` has no synchronous close, and this
    # runs from ``__init__``, which cannot await one.
    validate_credentials(
        client_id=client_id,
