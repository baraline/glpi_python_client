...
from glpi_python_client._sync.clients.commons._config import build_http_session
from glpi_python_client._sync.clients.commons._http import transport_error_from
...
def validate_credentials(
    *,
    ...
    """Check that the supplied OAuth credential sets are complete.

    GLPI authentication accepts client credentials, user credentials, or
    both. Partial pairs are rejected here so the token request path does not
    fail later with a less actionable error.

    This is a free function rather than only a method so callers can check a
    configuration *before* committing resources to it. That ordering matters:
    the client used to build its HTTP session first and unwind it in an
    ``except`` clause when validation failed, which is not expressible on the
    async surface -- an ``httpx.AsyncClient`` has no synchronous close, and a
    constructor cannot await one. Validating up front removes the need to
    unwind anything.

    Raises
    ------
    GlpiValidationError
        If either pair is half-supplied, or if neither pair is supplied.
    """
    ...
    if not (client_id is not None and client_secret is not None) and not (
