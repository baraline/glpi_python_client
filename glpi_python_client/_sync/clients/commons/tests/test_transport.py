...
def test_search_raises_on_4xx_instead_of_returning_empty(
    ...
    with pytest.raises(GlpiStatusError):
        retry_client.search_tickets()


def test_search_still_returns_empty_on_a_genuine_empty_result(
    retry_client: Any,
) -> None:
...
def test_iterators_surface_a_4xx_instead_of_ending_silently(
    retry_client: Any,
) -> None:
    ...
    with pytest.raises(GlpiStatusError):
        [batch for batch in retry_client.iter_search_tickets()]
