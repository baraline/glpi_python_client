...
def test_search_kb_articles_forwards_params(client: Any) -> None:
...
def test_get_kb_article_targets_per_id_endpoint(client: Any) -> None:
...
def test_create_kb_article_renders_markdown_and_returns_id(client: Any) -> None:
    rec = _Recorder()
    rec.install(client)
...
def test_update_kb_article_sends_patch(client: Any) -> None:
    rec = _Recorder()
    rec.install(client)
...
def test_delete_kb_article_with_force(client: Any) -> None:
    rec = _Recorder()
    rec.install(client)
...
def test_set_kb_article_categories_writes_via_v1(client: Any) -> None:
    fake = _FakeV1()
...
def test_set_kb_article_categories_empty_clears(client: Any) -> None:
    fake = _FakeV1()
    client._v1 = fake  # type: ignore[assignment]
...
def test_set_kb_article_categories_requires_v1(client: Any) -> None:
    assert client._v1 is None
    with pytest.raises(RuntimeError):
        client.set_kb_article_categories(31, [14])


def test_create_kb_article_applies_categories_via_v1(client: Any) -> None:
    rec = _Recorder()
    rec.install(client)
    fake = _FakeV1()
    client._v1 = fake  # type: ignore[assignment]
...
def test_create_kb_article_without_categories_skips_v1(client: Any) -> None:
    rec = _Recorder()
    rec.install(client)
...
def test_create_kb_article_ref_without_id_raises(client: Any) -> None:
...
def test_create_kb_article_empty_categories_skips_v1(client: Any) -> None:
    rec = _Recorder()
    rec.install(client)
    assert client._v1 is None  # no v1 configured
    new_id = client.create_kb_article(
...
def test_update_kb_article_applies_categories_via_v1(client: Any) -> None:
    rec = _Recorder()
    rec.install(client)
    fake = _FakeV1()
    client._v1 = fake  # type: ignore[assignment]
...
def test_update_kb_article_without_categories_skips_v1(client: Any) -> None:
    rec = _Recorder()
    rec.install(client)
    assert client._v1 is None
...
def test_iter_search_kb_articles_yields_every_page(client: Any) -> None:
    ...
    batches = [
        batch
        for batch in client.iter_search_kb_articles(
...
def test_iter_search_kb_articles_stops_on_a_single_short_page(
    client: Any,
) -> None:
    ...
    batches = [batch for batch in client.iter_search_kb_articles(batch_size=50)]

    assert starts == [0]
    assert len(batches) == 1


def test_iter_search_kb_articles_yields_nothing_when_empty(client: Any) -> None:
    ...
    assert [batch for batch in client.iter_search_kb_articles()] == []


def test_get_kb_article_localises_the_naive_revision_date(client: Any) -> None:
    ...
    article = client.get_kb_article(1)
