...
def test_search_entities_skips_entity_header(client: Any) -> None:
...
def test_get_entity_skips_entity_header(client: Any) -> None:
...
def test_update_entity_patch(client: Any) -> None:
...
def test_delete_entity_with_force(client: Any) -> None:
...
def test_create_entity_id_returned(client: Any) -> None:
...
def test_create_entity_skips_entity_header(client: Any) -> None:
...
def test_iter_search_entities_single_page(client: Any) -> None:
    ...
    batches = [b for b in client.iter_search_entities("", batch_size=50)]
    assert call_count == 1
...
def test_iter_search_entities_multi_page_stops_on_short_batch(
    ...
    client.search_entities = fake_search  # type: ignore[method-assign]
    batches = [batch for batch in client.iter_search_entities("", batch_size=2)]
    assert call_count == 2
