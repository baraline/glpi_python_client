...
def test_find_user_by_email_matches_a_nested_email_entry(client: Any) -> None:
    ...
    found = client.find_user_by_email("bob@x.test")

    assert found is not None
    assert found.id == 2


def test_find_user_by_email_is_case_insensitive(client: Any) -> None:
    ...
    found = client.find_user_by_email("  ALICE@x.TEST ")

    assert found is not None
    assert found.id == 1


def test_find_user_by_email_returns_none_when_absent(client: Any) -> None:
    ...
    assert client.find_user_by_email("nobody@x.test") is None


def test_find_user_by_email_scans_past_the_first_page(client: Any) -> None:
    ...
    found = client.find_user_by_email("zoe@x.test", batch_size=2)

    assert found is not None
    assert found.id == 9


def test_find_user_by_email_stops_at_the_matching_page(client: Any) -> None:
    ...
    client.find_user_by_email("alice@x.test", batch_size=2)

    assert starts == [0]


def test_find_user_by_email_scans_every_entity_by_default(client: Any) -> None:
    ...
    client.find_user_by_email("alice@x.test", rsql_filter="is_active==true")

    assert seen["skip_entity"] is True
    assert seen["filter"] == "is_active==true"


def test_find_user_by_email_rejects_a_blank_address(client: Any) -> None:
