...
def test_get_container_full_payload() -> None:
    ...
    payload = {
        "id": 10,
        "name": "aidelarsolution",
        "label": "Aide à la résolution",
        "itemtypes": '["Ticket"]',
        "type": "tab",
        "subtype": None,
        "entities_id": 0,
    ...
    assert container.name == "aidelarsolution"
...
def test_value_row_dynamic_columns_in_extra_payload() -> None:
    ...
    assert row.items_id == 62571
    assert row.extra_payload["aidelarsolutionfield"] == "<p>test</p>"
...
def test_post_value_row_carries_dynamic_columns() -> None:
    ...
    body = PostPluginFieldsValueRow(
        ...
        itemtype="Ticket",
        plugin_fields_containers_id=10,
        extra_payload={"aidelarsolutionfield": "<p>x</p>"},
    )
    assert body.extra_payload == {"aidelarsolutionfield": "<p>x</p>"}
