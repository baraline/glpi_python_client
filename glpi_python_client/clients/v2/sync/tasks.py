"""Synchronous task search operations for GLPI v2 clients.

This module contains the global task search helper used by the synchronous
high-level client.
"""
...
class SyncTaskMixin(SyncTransportMixin):
    """Synchronous GLPI task search helpers.

    The mixin exposes a typed global task search surface while keeping the
    pagination and endpoint-fallback rules out of the public client class.
    """
    ...
    def search_task_records(
        ...
        *,
    ...
    ) -> list[GlpiTask] | Iterator[list[GlpiTask]]:
        ...
        returns an iterator of typed task batches instead of one materialized
        list.
        """
    ...
    def _iter_task_record_batches(
        ...
        *,
        ...
        for page in self._yield_task_payloads(
            query=query,
            fields=fields,
            sort=sort,
            batch_size=batch_size,
        ...
        while True:
            ...
            if endpoint is None:
                for candidate in TASK_SEARCH_ENDPOINTS:
                    response = self._get_request(candidate, params)
