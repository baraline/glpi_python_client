"""Synchronous analytics and context helpers for GLPI v2 clients.

This module contains the higher-level aggregation helpers that compose the
existing public search and record-fetch methods.
"""
...
class SyncAnalyticsMixin(SyncTransportMixin):
    """Synchronous GLPI analytics and ticket-context helpers.

    These methods intentionally compose the already public low-level search and
    fetch helpers instead of bypassing them with ad hoc transport logic.
    """

    def get_ticket_context(self, ticket_id: str | int) -> GlpiTicketContext:
        """Return one grouped ticket context bundle.

        The bundle contains the primary ticket plus the common timeline and
        document records needed by higher-level workflows.
        """

        return GlpiTicketContext(
            ticket=self.get_ticket_record(ticket_id),
            tasks=self.get_task_records(ticket_id),
            followups=self.get_followup_records(ticket_id),
            solutions=self.get_solution_records(ticket_id),
            documents=self.get_document_records(ticket_id),
        )
    ...
    def get_task_durations(
        ...
        *,
        ...
        entity_ids, entity_labels = self._resolve_entity_scope(
            entity_id=entity_id,
    ...
    def get_ticket_statistics(
        ...
        *,
        ...
        tickets = self.search_ticket_records(query=query)
        filtered_tickets = [
            ticket
    ...
    def get_user_activity(
        ...
        *,
        ...
        tickets = self.search_ticket_records(
            query=build_date_range_filter("date_creation", start=start, end=end),
        )
        team_cache = {
            ticket.id: self.get_team_member_records(ticket.id)
            for ticket in tickets
    ...
    def _get_ticket_cache(self, tasks: list[GlpiTask]) -> dict[str, GlpiTicket]:
        ...
        for ticket_id in ticket_ids:
            if not isinstance(ticket_id, str):
                continue
            try:
                ticket_cache[ticket_id] = self.get_ticket_record(ticket_id)
            except ValueError:
                continue
        return ticket_cache
