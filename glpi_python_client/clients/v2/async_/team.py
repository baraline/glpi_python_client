"""Asynchronous team member operations for GLPI v2 clients.

This module provides the async ticket team-member helpers that list, add, and
remove assigned members through the GLPI API.
"""
...
class AsyncTeamMixin(AsyncTransportMixin):
    """Asynchronous GLPI ticket team member helpers.

    The mixin keeps team-member payload construction and response handling out
    of the public async client class.
    """

    async def get_team_member_records(self, ticket_id: GlpiId) -> list[GlpiTeamMember]:
        """Fetch the team members currently linked to one ticket.

        Non-success list responses are logged and normalized to an empty result
        list rather than raising immediately.
        """
    ...
    async def add_team_member(
        ...
        The typed member object is converted into the payload expected by GLPI,
        and the successful mutation is logged with the affected ticket.
        """
        ...
        response = await self._post_request(endpoint, payload)
    ...
    async def remove_team_member(
        ...
        The payload mirrors team-member creation so the same member model can be
        reused for add and remove workflows.
        """
        ...
        response = await self._delete_request(endpoint, payload)
        ensure_response_status(
            response,
