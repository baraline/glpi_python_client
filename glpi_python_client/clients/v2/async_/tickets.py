"""Asynchronous ticket operations for GLPI v2 clients.

This module contains the async ticket search, fetch, create, update, and
delete helpers used by the asynchronous high-level client.
"""
...
class AsyncTicketMixin(AsyncTransportMixin):
    """Asynchronous GLPI ticket search and mutation helpers.

    The mixin exposes the typed async ticket operations while delegating shared
    field and pagination rules to the common helper modules.
    """

    @overload
    async def search_ticket_records(
        self,
        query: str | None = None,
        *,
        fields: tuple[str, ...] = (),
    ...
    async def search_ticket_records(
        ...
        *,
        ...
        include_deleted_ticket: bool = False,
    ) -> list[GlpiTicket] | AsyncIterator[list[GlpiTicket]]:
        ...
        returns an async iterator of typed ticket batches instead of a fully
        materialized list.
        """
    ...
    async def _iter_ticket_record_batches(
        self,
        *,
        query: str | None = None,
        ...
        async for page in self._yield_ticket_payloads(
            query=query,
            fields=fields,
            sort=sort,
            batch_size=batch_size,
    ...
    async def get_ticket_record(
        ...
        *,
        ...
        stays aligned with the default async search behavior.
        """

        response = await self._get_request(f"{TICKET_ENDPOINT}/{ticket_id}")
    ...
    async def create_ticket(self, ticket: GlpiTicket) -> str:
        """Create a GLPI ticket and return the identifier assigned by GLPI.

        The ticket name precondition is enforced locally before the awaited
        request is sent, and the response must contain a created ticket ID.
        """
        ...
        response = await self._post_request(TICKET_ENDPOINT, payload_data)
        ensure_response_status(
            response,
    ...
    async def update_ticket(
        ...
        *,
        ...
        response = await self._update_request(
    ...
    async def delete_ticket(self, ticket_id: GlpiId) -> None:
        """Delete one GLPI ticket by identifier.

        The method logs successful deletion and returns ``None`` to match the
        package's async mutation-helper conventions.
        """

        response = await self._delete_request(f"{TICKET_ENDPOINT}/{ticket_id}")
        ensure_response_status(
            response,
            success_statuses=(200, 204),
    ...
    async def _yield_ticket_payloads(
        self,
        *,
        query: str | None = None,
        fields: list[str] | None = None,
        ...
        while True:
            ...
            response = await self._get_request(TICKET_ENDPOINT, params)
            if response.status_code not in (200, 206):
                logger.info(
