...
def test_async_search_task_records_returns_full_list_by_default(
    ...
    async def run_test() -> None:
        ...
        try:
            tasks = await client.search_task_records(
...
def test_async_search_task_records_returns_lazy_batches_when_requested(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def run_test() -> None:
        client = AsyncGlpiClient(
        ...
        monkeypatch.setattr(client, "_get_request", fake_get_request)
        try:
            batches = await client.search_task_records(batch_size=2)

            collected: list[list[str]] = []
            async for batch in batches:
                collected.append([task.task_id for task in batch])
