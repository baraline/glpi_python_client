"""Asynchronous GLPI v2 transport methods.

This module owns the awaitable transport helpers that wrap the shared
``requests`` session and keep blocking calls behind narrow thread boundaries.
"""
...
class AsyncTransportMixin:
    """Asynchronous GLPI API transport helpers.

    The transport mixin keeps token handling, header construction, retries, and
    request dispatch out of the endpoint-specific async mixins.
    """
    ...
    def _ensure_open(self) -> None:
        ...
        All async transport helpers call this guard before touching the shared
        HTTP session.
        """
    ...
    async def _ensure_token(self) -> None:
        ...
        Token refresh is serialized by the async lock so concurrent awaited
    ...
    async def _execute_request(
        ...
        *,
        ...
        placement before dispatching the blocking HTTP call through the async
        transport wrapper.
        """
