"""Asynchronous document operations for GLPI v2 clients.

This module covers async document listing, metadata lookup, binary download,
and deletion for ticket-linked GLPI documents.
"""
...
class AsyncDocumentMixin(AsyncTransportMixin):
    """Asynchronous GLPI ticket document helpers.

    These helpers keep document parsing and optional metadata enrichment aligned
    with the synchronous client while exposing awaitable operations.
    """
    ...
    async def get_document_records(
        ...
        *,
        ...
        async lookups and logs skipped enrichments without failing the whole
        batch.
        """
        ...
        for relation in timeline_payload_items(response.json()):
            ...
            if enrich_metadata:
                try:
                    document_record = await self.get_document_record(document_id)
                    metadata = {
    ...
    async def get_document_record(self, document_id: GlpiId) -> GlpiDocument:
    ...
    async def download_document_content(self, document_id: GlpiId) -> bytes:
        ...
        This helper returns raw response bytes and keeps the async content
        retrieval workflow aligned with the synchronous client.
        """
    ...
    async def delete_document(self, document_id: GlpiId) -> None:
        """Delete one GLPI document by identifier.

        Successful deletes return ``None`` and are logged for traceability.
        """
