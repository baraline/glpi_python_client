...
- Values are transmitted **verbatim -- there is no HTML/Markdown conversion on this path**, unlike ticket and KB article content. A `richtext` field takes raw HTML (`"<p>test</p>"`). Convert yourself if you want Markdown: `GlpiContentConverter` is not exported from the package root, import it from `glpi_python_client.content`. Calling it directly puts you on the same two rules the models get: HTML nested past `MAX_HTML_DEPTH` (200) comes back **tag-stripped rather than converted** (every character the normal rendering would have produced, none of the structure, no exception), and any other failure raises `GlpiContentError`.
