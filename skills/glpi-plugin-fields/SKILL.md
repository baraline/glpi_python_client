...
4. Read the stored row with `list_item_plugin_field_rows(itemtype, items_id, container_name)` -- positional arguments. It returns zero or one `GetPluginFieldsValueRow`; the values live in `row.extra_payload` and `row.id` is the `row_id` an update needs.
...
async def upsert_plugin_field_row(
    ...
    # Positional here; the two writers below are keyword-only.
    ...
    return await client.create_item_plugin_field_row(
        ...
        container_name=container_name,
        values=values,
        entities_id=0,  # create only; omitted from the body when not passed
    )
```
...
- A container that has never had a value saved for that ticket is **silently absent** from the `get_ticket_custom_fields` result -- you do not get `{"container": {}}`. Use `result.get(name, {})`, never `result[name]`. An empty overall dict means "no rows anywhere", not "no containers".
...
- Parameter-passing style is inconsistent across the family: `list_item_plugin_field_rows(itemtype, items_id, container_name)` takes **positional** arguments, while `create_item_plugin_field_row` and `update_item_plugin_field_row` are **keyword-only**. Calling either writer positionally is a `TypeError`.
