...
- HTML nested past `MAX_HTML_DEPTH` (200) is **stripped to text instead of converted**: `markdownify` recurses per level and dies around 494. Every character the normal rendering would have produced still appears, but structure does not: link targets, image alt text and code fencing are gone. Nothing raises. Any other conversion failure raises `GlpiContentError` -- from the `.content` read, not from the `list_*` call. Note that `GlpiTicketContext.to_markdown()` is usually the first thing to read every body, so it is where such an error surfaces.
