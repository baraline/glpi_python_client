...
- HTML nested past `MAX_HTML_DEPTH` (200) is **stripped to text instead of converted** -- every character the normal rendering would have produced, none of the Markdown structure, and no exception. Any other conversion failure raises `GlpiContentError`, from the `.content` read rather than from the search call.
