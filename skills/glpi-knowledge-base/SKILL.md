...
async def create_with_categories(
    client: AsyncGlpiClient, article: PostKBArticle, category_ids: list[int]
) -> int:
    try:
        ...
        await client.set_kb_article_categories(article_id, category_ids)
        return article_id
```
...
# A search never raises on a 4xx -- it returns []. Probe with a call that
# does raise if you need to tell 'no matches' from 'endpoint unavailable'.
if not faq:
    await client.get_kb_article(1)  # raises GlpiNotFoundError if absent
```
...
- `search_kb_articles` and `search_kb_categories` swallow 4xx and return `[]`; they pass no `failure_message`, so the status check is skipped. `list_kb_article_comments`, `list_kb_article_revisions` and `get_kb_article_revision` do pass one and raise `GlpiNotFoundError` normally. An empty list from a search can therefore mean "no matches", "bad RSQL filter", or "this GLPI serves no `/Knowledgebase` routes at all" (they need High-Level API >= 2.2.0); an empty list from a list helper is unambiguous.
...
- `GetKBArticle.revisions` and `.translations` hold private ref classes (leading underscore, not exported from the package root). Read their attributes (`.id`, `.revision`, `.language`, `.date`, `.name`); never import them.
